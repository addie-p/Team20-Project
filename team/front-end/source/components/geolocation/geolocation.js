export class GeolocationMapComponent {
    ...
    // use haversine formula and calculate distance between 2 points
    haversine(lat1, lon1, lat2, lon2) {
      ...
      return R * c; // resulting distance in km
    }
    ...
    // function to get the restaurant data from api 
    ...
    // initialize the mini map and add markers for resaurants
    async initializeMap() {
      ...
      if (!this.#map) {
        this.#map = L.map(mapElement).setView([this.latitude, this.longitude], this.zoomLevel); // Use mapElement here
    ...
    // add markers to the map without showing distances in the popup
addMarkers(map, selectedLat, selectedLon) {
    ...
    this.#restaurants.forEach((restaurant) => {
      ...
      // markers with name and cuisine
      L.marker([lat, lon]).addTo(map)
        .bindPopup(`<b>${name}</b><br><i>Cuisine: ${cuisine}</i>`);
  
      // add restaurant names with distance to list
      if (listContainer) {
    ...
    console.log('Rendered restaurant names, cuisines, and markers without distances on the map.');
  }
  
  
    // update location when user changes location
